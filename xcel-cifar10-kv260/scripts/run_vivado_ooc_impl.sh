...
LOG_BASENAME="${LOG_BASENAME:-vivado_ooc_impl}"
...
started_s="$(date +%s)"
echo "[vivado-ooc-impl] start=$(date -Is)"
set +e
vivado -mode batch -source "${SCRIPT_DIR}/vivado_ooc_impl.tcl" \
  -log "${PROJECT_DIR}/artifacts/logs/${LOG_BASENAME}.log" \
  -journal "${PROJECT_DIR}/artifacts/logs/${LOG_BASENAME}.jou"
status="$?"
set -e
ended_s="$(date +%s)"
echo "[vivado-ooc-impl] end=$(date -Is) status=${status} elapsed_sec=$((ended_s - started_s))"
exit "${status}"
