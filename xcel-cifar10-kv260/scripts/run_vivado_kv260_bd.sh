...
LOG_BASENAME="${LOG_BASENAME:-vivado_kv260_bd}"
...
started_s="$(date +%s)"
echo "[vivado-kv260-bd] start=$(date -Is)"
set +e
vivado -mode batch -source "${SCRIPT_DIR}/vivado_kv260_bd.tcl" \
  -log "${PROJECT_DIR}/artifacts/logs/${LOG_BASENAME}.log" \
  -journal "${PROJECT_DIR}/artifacts/logs/${LOG_BASENAME}.jou"
status="$?"
set -e
ended_s="$(date +%s)"
echo "[vivado-kv260-bd] end=$(date -Is) status=${status} elapsed_sec=$((ended_s - started_s))"
exit "${status}"
