...
{    
	...
	const std::streamsize expected = sizeof(unsigned char)*96*NUM_TESTS*32*32;
	if (!ifs_param) {
		cerr << "ERROR: cannot open conv1_input.bin from current working directory" << endl;
		exit(1);
	}
	ifs_param.read((char*)(images), expected);
	if (ifs_param.gcount() != expected) {
		cerr << "ERROR: conv1_input.bin is shorter than expected: read "
		     << ifs_param.gcount() << " bytes, expected " << expected << endl;
		exit(1);
	}
	ifs_param.close();
}
...
{    
	...
	const std::streamsize expected = sizeof(unsigned char)*NUM_TESTS;
	if (!ifs_param) {
		cerr << "ERROR: cannot open labels.bin from current working directory" << endl;
		exit(1);
	}
	ifs_param.read((char*)(labels), expected);
	if (ifs_param.gcount() != expected) {
		cerr << "ERROR: labels.bin is shorter than expected: read "
		     << ifs_param.gcount() << " bytes, expected " << expected << endl;
		exit(1);
	}
	ifs_param.close();
...
int main(int argc, char **argv)
{
	...
	int correct_hw = 0;
#ifndef LAYER_TEST
	int strict_hw_label_errors = 0;
	int strict_sw_hw_mismatches = 0;
#endif
...
//		}
		...
		int32 layer_trace[COSIM_TRACE_WORDS] = {0};
		FracNet_T(image_hw, accelerator_output, layer_trace);

		cout << "COSIM_LAYER_TRACE sample " << (k + 1) << endl;
		for (int layer = 0; layer < COSIM_TRACE_LAYER_COUNT; layer ++) {
			int base = layer*COSIM_TRACE_LAYER_WORDS;
			cout << "TRACE_LAYER " << layer_trace[base + 0]
			     << " sum=" << layer_trace[base + 1]
			     << " mix=" << layer_trace[base + 2]
			     << " s0=" << layer_trace[base + 3]
			     << " smid=" << layer_trace[base + 4]
			     << " slast=" << layer_trace[base + 5]
			     << endl;
		}
		cout << "TRACE_POOL";
		for (int i = 0; i < 64; i ++) {
			cout << " " << layer_trace[COSIM_TRACE_POOL_OFFSET + i];
		}
		cout << endl;
		cout << "TRACE_LINEAR";
		for (int i = 0; i < 10; i ++) {
			cout << " " << layer_trace[COSIM_TRACE_LINEAR_OFFSET + i];
		}
		cout << endl;
		for (int rec = 0; rec < COSIM_TRACE_FIRST_COUNT; rec ++) {
			int base = COSIM_TRACE_FIRST_OFFSET + rec*COSIM_TRACE_LAYER_WORDS;
			cout << "TRACE_FIRST " << layer_trace[base + 0]
			     << " sum=" << layer_trace[base + 1]
			     << " mix=" << layer_trace[base + 2]
			     << " s0=" << layer_trace[base + 3]
			     << " smid=" << layer_trace[base + 4]
...
//					if (err != 0) cout << "(" << i << ", " << j << ", " << k << ") ";
		...
		cout << "Sample " << (k + 1)
		     ...
		     << " sw_pred=" << predict
		     << " hw_pred=" << predict_hw
		     << " sw_correct=" << correct_sw
		     << " hw_correct=" << correct_hw
		     << endl;
		if (predict != predict_hw) {
			cout << "Prediction mismatch at sample " << (k + 1) << endl;
#ifdef STRICT_HW_CHECK
			strict_sw_hw_mismatches ++;
#endif
		}
#if defined(STRICT_HW_CHECK) && !defined(COSIM_LAYER_TRACE)
		if (predict_hw != labels[k]) {
			strict_hw_label_errors ++;
			cout << "Hardware label mismatch at sample " << (k + 1) << endl;
		}
#endif
...
#if defined(STRICT_HW_CHECK) && !defined(LAYER_TEST) && !defined(COSIM_LAYER_TRACE)
	if (strict_hw_label_errors != 0 || strict_sw_hw_mismatches != 0) {
		cerr << "ERROR: STRICT_HW_CHECK failed: "
		     << "hw_label_errors=" << strict_hw_label_errors
		     << " sw_hw_mismatches=" << strict_sw_hw_mismatches
		     << endl;
		return 1;
	}
#endif
